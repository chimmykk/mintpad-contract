...
contract MintpadERC1155Collection is ERC1155, Ownable {
    ...
    function mint(uint256 id, uint256 amount) external payable {
        ...
        if (currentMintPhase == MintPhase.Whitelist) {
            ...
            require(whitelistMinted[msg.sender] + amount <= whitelistMintLimit, "Whitelist mint limit reached.");
            whitelistMinted[msg.sender] += amount;
        } else if (currentMintPhase == MintPhase.Public) {
            require(publicMintLimit == 0 || balanceOf(msg.sender, id) + amount <= publicMintLimit, "Public mint limit reached.");
        } else {
            revert("Mint phase not set.");
        }
    ...
    function setMintPhase(
        ...
        require(_phaseSupply > 0, "Phase supply must be greater than zero.");
        require(_phaseSupply <= maxSupply - currentSupply, "Phase supply exceeds remaining supply.");
        require(_phaseMintPrice > 0, "Phase mint price must be greater than zero.");
        require(_phaseMintLimit > 0, "Phase mint limit must be greater than zero.");
