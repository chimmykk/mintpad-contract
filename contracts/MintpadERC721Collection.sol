...
contract MintpadERC721Collection is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 tokenId) external payable {
        ...
        if (currentMintPhase == MintPhase.Whitelist) {
            ...
            require(whitelistMinted[msg.sender] < whitelistMintLimit, "Whitelist mint limit reached.");
            whitelistMinted[msg.sender]++;
        } else if (currentMintPhase == MintPhase.Public) {
            require(publicMintLimit == 0 || balanceOf(msg.sender) < publicMintLimit, "Public mint limit reached.");
        } else {
    ...
    function setMintPhase(
        ...
        require(_phaseSupply <= maxSupply - totalSupply(), "Phase supply exceeds remaining supply.");
